...
const buildShared = {
	postConstructor(options) {
		const {name, stream, throughputWindow = 1000, copyKeys = [], skipUnwrap = false, timeout = null} = options;
		this.name = name;
		this.stream = stream;
		...
		this.stream.on('error', buildShared._onStreamError.bind(this));
	...
	_setThreadId(value, markers) {
		...
		if (this.timeout) {
			this.timeoutId = setTimeout(this._onTimeout.bind(this, markers), this.timeout);
		}
	...
	_onTimeout(markers) {
		console.log({markers});
		const error = new Error(`Tracify : Stream ${this.name} timeout after ${this.timeout} ms`);
		this.destroy(error);
	},
